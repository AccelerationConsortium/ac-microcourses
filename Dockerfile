...
# and `pip install ac-microcourses` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "ac-microcourses", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n ac-microcourses -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "ac-microcourses", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "ac-microcourses", "YOUR_CONSOLE_SCRIPT"]
