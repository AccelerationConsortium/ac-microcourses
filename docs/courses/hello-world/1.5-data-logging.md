...
```{caution}
I have intentionally and very purposefully chosen to expose the specific `LAMBDA_FUNCTION_URL` from above publicly and in plain-text, and without any additional authentication behind it. This is purely for the purpose of instruction and having a self-contained and functional example to broadly share without requiring setup on your part (i.e., an "early win"). In other words, unless you're teaching a large-scale course and have a strong reason to, **try to never expose sensitive info ike this publicly**. Likewise, for lambda functions specifically, there are additional restrictions that can be applied such as username/password authentication. I choose not to use this pattern because it's overkill. Likewise, it's only stored in plain text in a `my_secrets.py` file in this case because it's meant for a microcontroller running MicroPython, not a full operating system like Linux or Windows.
```
...
https://q.utoronto.ca/courses/367721/assignments/1499675?display=full_width_with_nav
:::
...
https://q.utoronto.ca/courses/367721/assignments/1499676?display=full_width_with_nav
:::
