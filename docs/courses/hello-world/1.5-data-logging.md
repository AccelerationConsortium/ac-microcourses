...
```{caution}
I have intentionally and very purposefully chosen to expose the specific `LAMBDA_FUNCTION_URL` from above publicly and in plain-text, without any additional authentication behind it. This is purely for the purpose of instruction and having a self-contained, functional example to help provide an "early win". Likewise, for lambda functions specifically, there are additional restrictions that can be applied such as username/password authentication, though I choose not to use this pattern in the module here because it's overkill. Likewise, it's only stored in plain text in a `my_secrets.py` file in this case because it's meant for a microcontroller running MicroPython, not a full operating system like Linux or Windows. Unless you're teaching a large-scale course and have a strong reason to, **try to never expose sensitive info ike this publicly**.
```
